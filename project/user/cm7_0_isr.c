...
void pit0_ch0_isr(void)
{
    ...
    // 2ms 进一次，这里每 10ms 做一次速度计算
    static uint16_t tick = 0;
    if (++tick < 5) return;
    tick = 0;

    // 读取两路增量
    int16_t e1 = encoder_get_count(LEFT_ENCODER_QD);   // E1 ← 左
    int16_t e2 = encoder_get_count(RIGHT_ENCODER_QD);  // E2 ← 右

    // 立刻清零硬件计数
    encoder_clear_count(LEFT_ENCODER_QD);
    encoder_clear_count(RIGHT_ENCODER_QD);

    // 应用固定符号，使“正占空比 → 速度为正”
    int32_t s1 = (int32_t)LEFT_ENCODER_SIGN  * e1;
    int32_t s2 = (int32_t)RIGHT_ENCODER_SIGN * e2;

    // 你的换算（保持原公式）
    Motor1Speed = (int)( s1 * 157 / (13 * 4 * 30) );   // 左轮速度
    Motor2Speed = (int)( s2 * 157 / (13 * 4 * 30) );   // 右轮速度

    // 开环 PWM（绕过速度 PID）—— duty=3 约为原 MIN_DUTY(5) 的一半
    {
        int d1 = (g_target_speed_m1 > 0.5f) ? 3 : 0;
        int d2 = (g_target_speed_m2 > 0.5f) ? 3 : 0;
        Motor_Set(d1, d2);
    }
}
void pit0_ch1_isr()                     // 定时器通道 1 周期中断服务函数      
{
    ...
    if (++hb >= 1000) { hb = 0; gpio_toggle_level(LED1); } // 2ms*1000 = 2000ms (2s)
}
